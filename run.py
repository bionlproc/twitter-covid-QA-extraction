...
positive_dev_ann = readJSONLine('golden/positive_sol.jsonl')
negative_dev_ann = readJSONLine('golden/negative_sol.jsonl')
can_not_test_dev_ann = readJSONLine('golden/can_not_test_sol.jsonl')
cure_dev_ann = readJSONLine('golden/cure_sol.jsonl')
death_dev_ann = readJSONLine('golden/death_sol.jsonl')

positive = readJSONLine('shared_task-test_set-final/shared_task-test-positive.jsonl')
can_not_test = readJSONLine('shared_task-test_set-final/shared_task-test-can_not_test.jsonl')
cure = readJSONLine('shared_task-test_set-final/shared_task-test-cure.jsonl')
death = readJSONLine('shared_task-test_set-final/shared_task-test-death.jsonl')
negative = readJSONLine('shared_task-test_set-final/shared_task-test-negative.jsonl')

positive_dev = make_test_dataset(positive, positive_dic)
can_not_test_dev = make_test_dataset(can_not_test, can_not_test_dic)
cure_dev = make_test_dataset(cure, cure_dic)
death_dev = make_test_dataset(death, death_dic)
negative_dev = make_test_dataset(negative, negative_dic)
...
raw_positive = readJSONLine('shared_task-test_set-final/shared_task-test-positive.jsonl')
raw_can_not_test = readJSONLine('shared_task-test_set-final/shared_task-test-can_not_test.jsonl')
raw_cure = readJSONLine('shared_task-test_set-final/shared_task-test-cure.jsonl')
raw_death = readJSONLine('shared_task-test_set-final/shared_task-test-death.jsonl')
raw_negative = readJSONLine('shared_task-test_set-final/shared_task-test-negative.jsonl')
...
if model_class == RobertaPrefixModelForQuestionAnswering:
    ...
    print(f'model_class = {str(model_class)[8:-2]}')
for epoch in range(epochs):
    ...
    if epoch == epochs - 1:
        save_in_json(positive_prediction, 'system_predictions/system_predictions-positive.jsonl')
        save_in_json(can_not_test_prediction, 'system_predictions/system_predictions-can_not_test.jsonl')
        save_in_json(cure_prediction, 'system_predictions/system_predictions-cure.jsonl')
        save_in_json(death_prediction, 'system_predictions/system_predictions-death.jsonl')
        save_in_json(negative_prediction, 'system_predictions/system_predictions-negative.jsonl')
    ...
    curr_result = runEvaluation(positive_prediction, positive_dev_ann)
    all_category_results[category_flag[0]] = curr_result
    curr_result = runEvaluation(can_not_test_prediction, can_not_test_dev_ann)
    all_category_results[category_flag[1]] = curr_result
    curr_result = runEvaluation(cure_prediction, cure_dev_ann)
    all_category_results[category_flag[2]] = curr_result
    curr_result = runEvaluation(death_prediction, death_dev_ann)
    all_category_results[category_flag[3]] = curr_result
    curr_result = runEvaluation(negative_prediction, negative_dev_ann)
