...
data_file_positive = 'dataset/positive-add_text.jsonl'
data_file_can_not_test = 'dataset/can_not_test-add_text.jsonl'
data_file_cure_and_prevention = 'dataset/cure_and_prevention-add_text.jsonl'
data_file_death = 'dataset/death-add_text.jsonl'
data_file_negative = 'dataset/negative-add_text.jsonl'

dataset_positive = read_jsonl_datafile(data_file_positive)
dataset_can_not_test = read_jsonl_datafile(data_file_can_not_test)
dataset_cure_and_prevention = read_jsonl_datafile(data_file_cure_and_prevention)
dataset_death = read_jsonl_datafile(data_file_death)
dataset_negative = read_jsonl_datafile(data_file_negative)
...
def make_all_dataset_in_tuple(task_questions_dict, task_instances_dict):
    all_data = []
    for task in list(task_instances_dict.keys()):
        positive = []
        negative = []
        curr_data = task_instances_dict[task]
        for sample in curr_data:
            tweet = sample[0]
            label = sample[-1]
            question = task_questions_dict[task]
            if label == 1:
                positive.append(tweet)
                if sample[1] != 'AUTHOR OF THE TWEET':
                    answer = {'text': tweet[sample[3]:sample[4]], 'answer_start': sample[3], 'answer_end': sample[4]}
                else:
                    answer = {'text': sample[1], 'answer_start': sample[3], 'answer_end': sample[4]}
                tup = (tweet, question, answer, label, task)
                all_data.append(tup)
        for sample in curr_data:
            tweet = sample[0]
            label = sample[-1]
            question = task_questions_dict[task]
            if label == 0 and tweet not in positive:
                if tweet not in negative:
                    negative.append(tweet)
                    answer = {'text': '', 'answer_start': -1, 'answer_end': -1}
                    tup = (tweet, question, answer, label, task)
                    all_data.append(tup)
    return all_data


# make training data for positive event
questions = [
    'What is the age of the person?',
    'Who is in close contact?',
    'Who is the employer?',
    'Is the gender male?',
    'Is the gender female?',
    'Who is tested positive?',
    'Where did the person recently visit?',
    'Does the person have a relationship?',
    'When is the cases reported?',
    'Where is the cases reported?']

task_instances_dict, tag_statistics, question_keys_and_tags = make_instances_from_dataset(dataset_positive)
task_questions_dict = {list(task_instances_dict.keys())[i]: questions[i] for i in range(len(questions))}
positive = make_all_dataset_in_tuple(task_questions_dict, task_instances_dict)

# make training data for negative event
questions = [
    'What is the age of the person?',
    'Who is in close contact?',
    'Is the gender male?',
    ...
    'When is the cases reported?',
    'Where is the cases reported?']
task_instances_dict, tag_statistics, question_keys_and_tags = make_instances_from_dataset(dataset_negative)
task_instances_dict.pop('how_long')
task_questions_dict = {list(task_instances_dict.keys())[i]: questions[i] for i in range(len(questions))}
negative = make_all_dataset_in_tuple(task_questions_dict, task_instances_dict)

# make dataset for can not test event
questions = [
    'Who can not get a test?',
    'Does the person have a relationship?',
    'Is the person experiencing any symptoms?',
    'When is the situation reported?',
    'Where is the situation reported?'
]
task_instances_dict, tag_statistics, question_keys_and_tags = make_instances_from_dataset(dataset_can_not_test)
task_questions_dict = {list(task_instances_dict.keys())[i]: questions[i] for i in range(len(questions))}
can_not_test = make_all_dataset_in_tuple(task_questions_dict, task_instances_dict)

# make dataset for cure event
questions = [
    'Does the author believe the method?',
    'What is the cure?',
    'Who is promoting the cure?'
]
task_instances_dict, tag_statistics, question_keys_and_tags = make_instances_from_dataset(dataset_cure_and_prevention)
task_questions_dict = {list(task_instances_dict.keys())[i]: questions[i] for i in range(len(questions))}
cure = make_all_dataset_in_tuple(task_questions_dict, task_instances_dict)

# make dataset for death event
questions = [
    'What is the age of the person?',
    'Who is dead?',
    'Does the person have a relationship?',
    'When is the case reported?',
    'Where is the case reported?']
task_instances_dict, tag_statistics, question_keys_and_tags = make_instances_from_dataset(dataset_death)
task_instances_dict.pop('symptoms')
task_questions_dict = {list(task_instances_dict.keys())[i]: questions[i] for i in range(len(questions))}
death = make_all_dataset_in_tuple(task_questions_dict, task_instances_dict)

dataset = positive + negative + can_not_test + cure + death

id_instance = {}
i = 0
added_id = []
for instance in dataset:
    id_instance[i] = tuple(list(instance)+[str(i)])
    added_id.append(tuple(list(instance) + [str(i)]))
    i += 1
...
# save all the dataset to pickle as training
save_in_pickle(added_id, 'train_dataset.pkl')